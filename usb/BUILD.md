...
**To build an ISO instead of writing to USB** (for VM testing), open PowerShell as Administrator, `cd` into the `usb\` folder, then run:
```powershell
cd C:\path\to\near-zero-touch-autopilot-enrollment-main\usb
powershell -ExecutionPolicy Bypass -File build.ps1 -Iso
```
The ISO is saved as `AutopilotEnrollment_amd64.iso` in that same folder.
